...
<!-- /新增：全部删除按钮区域 -->
  ...
  <script>
    // #################################################################
    // ###########      START: FULL & CORRECTED JAVASCRIPT     ###########
    // #################################################################
    ...
    async function loadMachinesFromBackend() {
        try {
            ...
            // 关键修复：将后端的 snake_case (machine_id) 映射到前端需要的 camelCase (machineId)
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      try {
        ...
        // +++ FIX: 移除localStorage依赖，直接硬编码默认员工列表，保证加载成功 +++
        workers = [
    ...
    function addNewWorker() { const workerId = document.getElementById('newWorkerId').value.trim(); const name = document.getElementById('newWorkerName').value.trim(); if (!workerId || !name) { showAlert('请输入工号和姓名', 'danger'); return; } if (workers.some(w => w.workerId === workerId)) { showAlert(`工号 ${workerId} 已存在`, 'warning'); return; } const newWorker = { workerId, name }; workers.push(newWorker); updateWorkersTable(); updateWorkerSelect(); updateWorkerStats(); showAlert(`成功添加员工：${name} (${workerId})`, 'success'); document.getElementById('newWorkerId').value = ''; document.getElementById('newWorkerName').value = ''; }
    function deleteWorker(workerId) { if (confirm('确定要删除该员工吗？')) { workers = workers.filter(w => w.workerId !== workerId); updateWorkersTable(); updateWorkerSelect(); updateWorkerStats(); showAlert('员工已删除', 'success'); } }
